...
Task 013에서 Pyshia가 `jvm.metrics.raw`, `http.metrics.raw`, `hikari.metrics.raw` 세 토픽을 소비하는 진입점(`*MetricSnapshotConsumer` → `*MetricSnapshotHandler`)까지 구축했으나, 각 Handler는 hook 주석으로 비어있다. 후속 Task(임계값 평가 / 알림 / 상태 저장)가 일관된 기준으로 동작하려면 "어떤 메트릭이 어떤 값일 때 위험인지" 가 합의된 정의로 먼저 존재해야 한다. 본 task는 코드 변경 없이 그 정의 문서만 산출한다.
...
### 수정하지 않는 파일
- 모든 `.java` 소스 (정의 task)
- `pyshia/src/main/resources/application.yml`
...
```
[Argus]                         [Kafka]                  [Pyshia]                          [후속 Task: 평가/알림]
...
- `C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\dto\jvm\*.java` — JVM DTO 필드명/단위 확인
- `C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\dto\http\*.java` — HTTP DTO 필드명/단위 확인
- `C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\dto\hikari\*.java` — Hikari DTO 필드명/단위 확인
...
- Task: 후속 권장 PromQL 추가 (Argus assembler/DTO + Pyshia DTO 동시 변경)
