# Task 017: Pyshia 메트릭 RDB 저장 — 구현 계획

## Context

Task 013에서 Pyshia가 jvm.metrics.raw / http.metrics.raw / hikari.metrics.raw Kafka 토픽을 수신하고 Handler에서 단순 검증 후 ThresholdEvaluator로 위임하는 구조를 갖추었다. 그러나 수신한 스냅샷은 임계값 평가에만 쓰일 뿐 영속화되지 않아 후속 Task(LLM 분석, 이력 조회, 패턴 학습 등)에서 과거 메트릭을 사용할 수 없다.
...
| 결정 | 선택 | 이유 |
|------|------|------|
| 패키지 | com.example.pyshia.metric (top-level) | alert, email, kafka와 같은 결. 저장 책임을 단일 패키지에 응집. 하위에 domain / repository / service 분리 (CLAUDE.md 패키지 규칙) |
...
| schema.sql (옵션) | pyshia/src/main/resources/db/schema-postgres.sql (참고용) | 운영 환경에서 ddl-auto를 끄고 수동 적용할 때 사용. spring.sql.init.mode=never 기본 유지로 자동 실행은 X. 본 task에서는 파일 생성만, 실 적용 옵션은 후속 |
...
**JvmMetricSnapshotEntity** (com.example.pyshia.metric.domain) — @Entity @Table(name="jvm_metric_snapshot")
...
HttpMetricSnapshotEntity (com.example.pyshia.metric.domain) — @Entity @Table(name="http_metric_snapshot")
...
HikariMetricSnapshotEntity (com.example.pyshia.metric.domain) — @Entity @Table(name="hikari_metric_snapshot")
...
HttpEndpointMetricPointEntity (com.example.pyshia.metric.domain) — @Entity @Table(name="http_endpoint_metric_point")
- Long id (IDENTITY)
...
HikariPoolMetricPointEntity (com.example.pyshia.metric.domain) — @Entity @Table(name="hikari_pool_metric_point")
...
- com.example.pyshia.metric.repository.JvmMetricSnapshotRepository — extends JpaRepository<JvmMetricSnapshotEntity, Long> (조회 메서드 0개)
- com.example.pyshia.metric.repository.HttpMetricSnapshotRepository — extends JpaRepository<HttpMetricSnapshotEntity, Long>
- com.example.pyshia.metric.repository.HikariMetricSnapshotRepository — extends JpaRepository<HikariMetricSnapshotEntity, Long>

자식 포인트 Repository는 cascade로 부모와 함께 저장되므로 불필요(저장 only). 후속 조회 task에서 추가.

### Service

MetricStoreService (com.example.pyshia.metric.service) — interface
...
MetricStoreServiceImpl (com.example.pyshia.metric.service) — @Service @RequiredArgsConstructor
...
MetricStoreErrorCode (com.example.pyshia.metric.exception) — enum implements ErrorCode
...
MetricStoreException (com.example.pyshia.metric.exception) — extends CustomException, Throwable cause까지 보존하는 생성자 사용
...
pyshia/src/main/resources/application.yml (수정) — 다음 키 추가
- spring.datasource.url: jdbc:postgresql://localhost:5432/pyshia
- spring.datasource.username: ${DB_USERNAME:pyshia}
- spring.datasource.password: ${DB_PASSWORD:pyshia}
...
pyshia/src/main/resources/application-test.yml (신규)
- spring.datasource.url: jdbc:h2:mem:pyshia;MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE
- spring.datasource.driver-class-name: org.h2.Driver
- spring.datasource.username: sa
- spring.jpa.hibernate.ddl-auto: create-drop
- spring.jpa.properties.hibernate.dialect: org.hibernate.dialect.H2Dialect

pyshia/src/main/resources/db/schema-postgres.sql (신규, 옵션 — 운영 수동 적용용)
...
- 서비스 1개: postgres:16-alpine, port 5432, volume postgres_data, env POSTGRES_DB=pyshia / POSTGRES_USER=pyshia / POSTGRES_PASSWORD=pyshia
- 기존 docker/docker-compose.kafka.yml과 동급으로 분리 (CLAUDE.md "Docker 파일 규칙: /docker 경로에 생성" 준수)

### 수정 대상 파일

- com.example.pyshia.kafka.consumer.JvmMetricSnapshotHandler — MetricStoreService 생성자 주입 추가, evaluator.evaluateJvm(snapshot) 호출 후 try { metricStoreService.save(snapshot); } catch (MetricStoreException e) { log.error(...); } 추가
- com.example.pyshia.kafka.consumer.HttpMetricSnapshotHandler — 동일 패턴
- com.example.pyshia.kafka.consumer.HikariMetricSnapshotHandler — 동일 패턴
...
- pyshia/src/test/java/.../metric/service/MetricStoreServiceImplTest.java (Mockito)
...
- pyshia/src/test/java/.../metric/service/MetricStoreServiceImplPersistenceTest.java (@DataJpaTest + H2)
...
- pyshia/src/test/java/.../kafka/consumer/JvmMetricSnapshotHandlerTest.java (보강)
...
### 컨텍스트 로딩 검증
- 기존 PyshiaApplicationTests 통과 (test profile -> H2로 fallback)
- 본 task에서 Spring Boot test를 띄울 때 @ActiveProfiles("test") 적용

### 통합 검증 (선택, 본 task 권장)
- 로컬 PostgreSQL 기동 후 ./gradlew :pyshia:bootRun
...
신규 (절대 경로):
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\domain\JvmMetricSnapshotEntity.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\domain\HttpMetricSnapshotEntity.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\domain\HikariMetricSnapshotEntity.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\domain\HttpEndpointMetricPointEntity.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\domain\HikariPoolMetricPointEntity.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\domain\HttpMetricKind.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\domain\HikariMetricKind.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\repository\JvmMetricSnapshotRepository.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\repository\HttpMetricSnapshotRepository.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\repository\HikariMetricSnapshotRepository.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\service\MetricStoreService.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\service\MetricStoreServiceImpl.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\exception\MetricStoreErrorCode.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\metric\exception\MetricStoreException.java
- C:\side_project\pyshia\src\main\resources\application-test.yml
- C:\side_project\pyshia\src\main\resources\db\schema-postgres.sql (옵션, 운영 수동 적용용)
- C:\side_project\docker\docker-compose.postgres.yml
- C:\side_project\pyshia\src\test\java\com\example\pyshia\metric\service\MetricStoreServiceImplTest.java
- C:\side_project\pyshia\src\test\java\com\example\pyshia\metric\service\MetricStoreServiceImplPersistenceTest.java

수정 (절대 경로):
- C:\side_project\pyshia\build.gradle — JPA, postgresql driver, h2 testRuntime 추가
- C:\side_project\pyshia\src\main\resources\application.yml — spring.datasource, spring.jpa 추가
- C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\consumer\JvmMetricSnapshotHandler.java — Store 주입 + 호출 + try-catch
...
- C:\side_project\pyshia\src\test\java\com\example\pyshia\kafka\consumer\JvmMetricSnapshotHandlerTest.java — Store mock + verify 보강
...
- C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\dto\jvm\*.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\dto\http\*.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\dto\hikari\*.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\dto\SnapshotStatus.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\dto\MetricStatus.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\common\exception\CustomException.java
- C:\side_project\pyshia\src\main\java\com\example\pyshia\common\exception\ErrorCode.java
