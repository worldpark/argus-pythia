# Task 015: Pythia Email Alarm 구현 — 구현 계획

## Context

Pythia는 Task 013에서 Kafka 메트릭 스냅샷 수신 진입점을, Task 014에서 임계값 정의 문서를 갖췄으나, 임계값 위반 시 운영자에게 알릴 수단이 전혀 없다. `spring-boot-starter-mail` 의존도 없고 `email`/`alarm` 패키지도 존재하지 않으며, CLAUDE.md가 규정한 공통 `CustomException` 부모 클래스 역시 아직 미구현 상태다.
...
| 패키지 위치 | **`com.example.pythia.email`** (top-level) | Constraint("별도 email 패키지 생성") 명시. CLAUDE.md 패키지 구조에서 `kafka/`도 top-level로 다뤄지는 것과 동일한 결 |
...
| 수신자/발신자 관리 | **`@ConfigurationProperties("pythia.email")` → `EmailProperties` record** | yml 키 변경 시 컴파일 타임 검출 가능. `@Value` 산발 주입 회피. `List<String>` 다중 수신자 자연스럽게 처리 |
...
**`com.example.pythia.common.exception.CustomException`** — abstract class, RuntimeException 상속
- 필드: `ErrorCode errorCode`
- 생성자: `(ErrorCode, String message)`, `(ErrorCode, String message, Throwable cause)`
- getter: `getErrorCode()`

**`com.example.pythia.common.exception.ErrorCode`** — interface
- `String code()`
- `String defaultMessage()`
- 다른 도메인이 enum으로 구현. 본 task에서는 EmailErrorCode 1개 도입

**`com.example.pythia.email.EmailService`** — `@Service`
...
**`com.example.pythia.email.dto.EmailRequest`** — record
- `List<String> to`, `String subject`, `String body`
- 정합성 검증(빈 to/subject/body 체크)은 EmailService 진입 시점

**`com.example.pythia.email.config.EmailProperties`** — `@ConfigurationProperties(prefix = "pythia.email")`, record 또는 `@Validated` 클래스
- `String from`
- `List<String> operatorRecipients`

**`com.example.pythia.email.exception.EmailSendException`** — `extends CustomException`
- 생성자: `(EmailErrorCode, String message, Throwable cause)`

**`com.example.pythia.email.exception.EmailErrorCode`** — enum, `implements ErrorCode`
- 후보 값:
...
- 기존 DTO들 — 무관
- `PythiaApplication.java` — `@ConfigurationPropertiesScan` 추가가 필요할 수 있음(EmailProperties 활성화). 자동 구성으로 안 잡히면 **부팅 클래스에 `@ConfigurationPropertiesScan` 한 줄만 추가** — 다른 변경 없음
...
| 부팅 | `pythia.email.operator-recipients` 비어있음 | EmailProperties에 `@NotEmpty` 검증 또는 EmailService 첫 호출 시 IllegalState. 결정: 부팅 시 `@Validated` + `@NotEmpty`로 fail-fast (운영 안전) |
...
- `./gradlew :pythia:build` BUILD SUCCESSFUL — 의존 추가/패키지 신설로 컴파일/컨텍스트 로딩 깨지지 않는지
- 기존 `PythiaApplicationTests` 컨텍스트 로딩 통과 (단, `MAIL_USERNAME/MAIL_PASSWORD` 환경 변수가 없으면 ApplicationContext 로딩 실패 가능 → 검증용 더미 값 환경에 설정하거나 test profile에서 `spring.mail.*` 오버라이드)

### 수동 통합 검증 (선택)
- 실제 Gmail 계정 + 앱 비밀번호로 환경 변수 설정 → Pythia 부팅 → Spring Boot Actuator 또는 한 회성 Runner로 `EmailService.sendToOperator("test", "hello")` 호출 → 운영자 메일 수신 확인
...
신규/수정 (절대 경로):
- `C:\side_project\pythia\build.gradle` — spring-boot-starter-mail 의존 추가
- `C:\side_project\pythia\src\main\resources\application.yml` — `spring.mail.*` + `pythia.email.*` 키 추가
- `C:\side_project\pythia\src\main\java\com\example\pythia\PythiaApplication.java` — `@ConfigurationPropertiesScan` 추가 (한 줄)
- `C:\side_project\pythia\src\main\java\com\example\pythia\common\exception\CustomException.java` — 신규 (abstract, RuntimeException 상속)
- `C:\side_project\pythia\src\main\java\com\example\pythia\common\exception\ErrorCode.java` — 신규 (interface)
- `C:\side_project\pythia\src\main\java\com\example\pythia\email\EmailService.java` — 신규 (`@Service`)
- `C:\side_project\pythia\src\main\java\com\example\pythia\email\dto\EmailRequest.java` — 신규 (record)
- `C:\side_project\pythia\src\main\java\com\example\pythia\email\config\EmailProperties.java` — 신규 (`@ConfigurationProperties`)
- `C:\side_project\pythia\src\main\java\com\example\pythia\email\exception\EmailSendException.java` — 신규
- `C:\side_project\pythia\src\main\java\com\example\pythia\email\exception\EmailErrorCode.java` — 신규 (enum)

참조 (수정 없음):
- `C:\side_project\pythia\CLAUDE.md` — 패키지/예외/Kafka 규약 정합성 확인
- `C:\side_project\pythia\src\main\java\com\example\pythia\kafka\consumer\*Handler.java` — 후속 트리거 task의 호출 지점 후보 (본 task 수정 X)
