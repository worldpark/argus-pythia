# Task 013: Pyshia Kafka Consumer 환경 구성 — 구현 계획

## Context

Argus는 Task 007/012로 `jvm.metrics.raw`, `http.metrics.raw`, `hikari.metrics.raw` 세 토픽에 `JacksonJsonSerializer` 기반 메시지를 발행하는 파이프라인을 갖췄으나, 수신 측인 Pyshia에는 아직 Kafka Consumer 환경이 없다. Pyshia에는 Task 011 미러링 결과로 `kafka.dto.{jvm,http,hikari}` 패키지에 세 DTO record가 이미 자리잡았지만(`build.gradle`에 spring-kafka 미추가, `application.yml`은 `spring.application.name`만 존재), 메시지를 받아들일 listener·config·역직렬화 설정이 모두 비어있다. 본 task는 이 빈 자리를 채워 세 토픽이 정상적으로 소비되는 출발점(=후속 임계값/룰 처리의 진입점)을 마련한다.
...
| 역직렬화 | **`JacksonJsonDeserializer<T>` (타입 명시 생성자)** | Pyshia CLAUDE.md 규칙: Spring Boot 4 환경에서 `JsonDeserializer` deprecated → `JacksonJsonDeserializer` 사용. Producer가 `spring.json.add.type.headers: false`로 발행하므로 헤더 의존 없이 타입 강제 가능 |
...
| group-id | **`spring.kafka.consumer.group-id: pyshia`** | 단일 그룹으로 시작 — 동일 인스턴스가 모든 토픽 파티션을 함께 받음. 추후 도메인별 분리 필요 시 `@KafkaListener(groupId=...)` 오버라이드 가능 |
...
| 패키지 위치 | **`com.example.pyshia.kafka.consumer`** | Task 제약("별도의 consumer 패키지 생성")에 맞춤. 기존 DTO 패키지(`kafka.dto`)와 동일 루트(`kafka`) 아래 형제 패키지로 위치 |
| Config 위치 | **`com.example.pyshia.kafka.config`** | DTO/Consumer와 한 루트(`kafka`) 아래 모음. 추후 Producer가 추가될 일은 거의 없으나 패키지 분리는 책임 명확 |
...
**`com.example.pyshia.kafka.config.KafkaConsumerConfig.java` — `@Configuration`**
...
**`com.example.pyshia.kafka.consumer.JvmMetricSnapshotConsumer.java` — `@Component`**
- `@KafkaListener(topics = "${pyshia.kafka.topic.jvm-metrics-raw}", containerFactory = "jvmKafkaListenerContainerFactory")`
...
**`com.example.pyshia.kafka.consumer.HttpMetricSnapshotConsumer.java` — `@Component`** — 동일 구조, `http.metrics.raw` / Http 팩토리 / HttpDto

**`com.example.pyshia.kafka.consumer.HikariMetricSnapshotConsumer.java` — `@Component`** — 동일 구조, `hikari.metrics.raw` / Hikari 팩토리 / HikariDto

### 수정하지 않는 파일 (제약/안전)
- 기존 `com.example.pyshia.kafka.dto.**` — Task 011 결과물 그대로 사용
- `PyshiaApplication.java` — `@SpringBootApplication`만으로 `@KafkaListener` 자동 활성화(`@EnableKafka`는 Spring Boot 자동 구성에서 처리)
...
```
[Argus]                                      [Kafka Broker @ localhost:9092]                  [Pyshia]
...
스프링 자동 구성이 `KafkaProperties`를 읽어 default `ConsumerFactory`를 만들지만, 본 설계는 타입별 팩토리를 **명시 지정**(`@KafkaListener(containerFactory=...)`)하므로 default factory 경로를 타지 않는다. 결과적으로 한 Pyshia 인스턴스가 세 토픽의 파티션을 동시에 폴링하고, 메시지마다 컨테이너 팩토리 → DTO 디시리얼라이저 → 리스너 메서드 순으로 흐른다.
...
| 설정 누락 | `pyshia.kafka.topic.*` 키가 없음 | `@KafkaListener(topics="${...}")`의 SpEL 해석 실패 → 부팅 실패 (fail-fast 의도) |
...
- 수동: Argus를 같은 broker(localhost:9092)에 띄우고 Pyshia 부팅 → Pyshia 로그에서 세 토픽의 debug 라인이 60초 주기로 출력되는지 확인
...
4. **단일 group-id `pyshia` vs 토픽별 group-id**: 단일이면 운영 단순하지만, 한 도메인의 backpressure가 다른 도메인 폴링 지연으로 번질 수 있음. 본 task 트래픽(60초 주기 × 3토픽)에선 무시 가능 → 단일 채택. 추후 트래픽 증가 시 분리.
...
신규/수정 (절대 경로):
- `C:\side_project\pyshia\build.gradle` — spring-kafka 의존 추가
- `C:\side_project\pyshia\src\main\resources\application.yml` — kafka 속성 + 토픽 키 3개
- `C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\config\KafkaConsumerConfig.java` — 신규
- `C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\consumer\JvmMetricSnapshotConsumer.java` — 신규
- `C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\consumer\HttpMetricSnapshotConsumer.java` — 신규
- `C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\consumer\HikariMetricSnapshotConsumer.java` — 신규
- `C:\side_project\pyshia\src\test\java\com\example\pyshia\kafka\consumer\JvmMetricSnapshotConsumerTest.java` — 신규
- `C:\side_project\pyshia\src\test\java\com\example\pyshia\kafka\consumer\HttpMetricSnapshotConsumerTest.java` — 신규
- `C:\side_project\pyshia\src\test\java\com\example\pyshia\kafka\consumer\HikariMetricSnapshotConsumerTest.java` — 신규

참조 (수정 없음):
- `C:\side_project\pyshia\src\main\java\com\example\pyshia\kafka\dto\jvm\JvmMetricSnapshotDto.java` 외 두 DTO — Task 011 산출물
