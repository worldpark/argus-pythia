...
class JvmMetricSnapshotAssemblerTest {
    ...
    void setUp() {
        ...
        assembler = new JvmMetricSnapshotAssembler(queryService, clock, fanoutExecutor);
    }

    @AfterEach
    void tearDown() {
        fanoutExecutor.close();
    }
    ...
    void assemble_allFail_returnsFailedSnapshot() {
        ...
        when(queryService.queryByMetric(MetricType.HEAP_USAGE))
            .thenThrow(new PrometheusQueryException("network error"));
        when(queryService.queryByMetric(MetricType.HEAP_OLD_GEN_USAGE))
            .thenThrow(new PrometheusQueryException("network error"));
        ...
        when(queryService.queryByMetric(MetricType.ACTIVE_THREADS))
            .thenThrow(new PrometheusQueryException("network error"));
        when(queryService.queryByMetric(MetricType.PEAK_THREADS))
            .thenThrow(new PrometheusQueryException("network error"));
        when(queryService.queryByMetric(MetricType.DAEMON_THREADS))
            .thenThrow(new PrometheusQueryException("network error"));
    ...
    @Test
    void assemble_runsQueriesOnVirtualThreads() {
        List<Boolean> capturedVirtual = Collections.synchronizedList(new ArrayList<>());
        PrometheusResponse successResponse = buildSuccessResponse("myapp", "host:8080", "0.5");

        doAnswer(invocation -> {
            capturedVirtual.add(Thread.currentThread().isVirtual());
            return successResponse;
        }).when(queryService).queryByMetric(any(MetricType.class));

        assembler.assemble();

        assertThat(capturedVirtual).isNotEmpty();
        assertThat(capturedVirtual).allMatch(isVirtual -> isVirtual);
    }

    private PrometheusResponse buildSuccessResponse(String application, String instance, String value) {
