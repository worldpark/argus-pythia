...
public sealed interface HikariMetricResult
    permits HikariActiveDto, HikariPendingDto {
  ...
  static <T extends HikariMetricResult> T from(
      MultiMappingResult r,
      BiFunction<List<PoolMetricPointDto>, OffsetDateTime, T> onSuccess,
      Supplier<T> onEmpty,
      Function<String, T> onParseFailed,
      Function<String, T> onQueryFailed) {
    if (r instanceof MultiMappingResult.Success s) {
      List<PoolMetricPointDto> points = s.points().stream()
          .map(p -> new PoolMetricPointDto(p.identifier(), p.value(), p.timestamp()))
          .toList();
