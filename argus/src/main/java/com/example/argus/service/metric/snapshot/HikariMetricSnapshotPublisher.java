...
public class HikariMetricSnapshotPublisher {
    ...
    public CompletableFuture<SendResult<String, HikariMetricSnapshotDto>> publish() {
        ...
        future.whenComplete((result, ex) -> {
            ...
            if (ex != null) {
                log.warn("metric-publish failed: metricType=HIKARI serviceId={} elapsedMs={}", serviceId, elapsedMs, ex);
                try {
                ...
                log.info("metric-publish completed: metricType=HIKARI serviceId={} elapsedMs={}", serviceId, elapsedMs);
            }
        });
        return future;
    }
